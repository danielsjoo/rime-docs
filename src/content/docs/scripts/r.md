...
> The default entrypoint is `run`. `transform` is also accepted for compatibility.
> To use a different name, set `entrypoint:` on the node.
...
`in_slots` and `out_slots` in `rime::register(...)` are how R communicates the schema back to the runtime. Use `"table"` for tibbles and `"any"` for non-tabular values.
...
- [Python language nodes](/rime-docs/scripts/python/) — same protocol, pandas instead of tibble
